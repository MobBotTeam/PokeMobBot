...
namespace PoGo.PokeMobBot.CLI
{
    public class ConsoleEventListener
    {
        ...
        public void HandleEvent(PlayerLevelUpEvent evt)
        {
            _logger.Write(
                _translation.GetTranslation(TranslationString.EventLevelUpRewards, evt.Items),
                LogLevel.Info);
        }
        ...
        public void HandleEvent(DisplayHighestsPokemonEvent evt)
        {
            ...
            _logger.Write($">  {"CP/BEST".PadLeft(8, ' ')}{(evt.DisplayPokemonMaxPoweredCp ? "/POWERED" : "")} |\t{strPerfect.PadLeft(6, ' ')}\t| LVL | {strName.PadRight(10, ' ')} | {("MOVE1").PadRight(18, ' ')} | {("MOVE2").PadRight(6, ' ')} {(evt.DisplayPokemonMovesetRank ? "| MoveRankVsAveType |" : "")}", LogLevel.Info, ConsoleColor.Yellow);
