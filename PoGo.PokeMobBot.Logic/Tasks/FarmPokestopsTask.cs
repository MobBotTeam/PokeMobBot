...
namespace PoGo.PokeMobBot.Logic.Tasks
{
    public static class FarmPokestopsTask
    {
        ...
        public static async Task Execute(ISession session, CancellationToken cancellationToken)
        {
            ...
            //This can likely be cleaned up in the future.
            if (session.LogicSettings.Teleport)
            {
                //So basically I have no idea how to properly name this as a setting so it won't confuse users so it's just sitting here like this.
                var stopsToHit = 20;
                //but we're going to hit 20 stops refreshing the closest stop
                //session.LogicSettings.MaxTravelDistanceInMeters effectively becomes max teleport distance when using this.
                cancellationToken.ThrowIfCancellationRequested();


                var stopsHit = 0;

                while (stopsHit < stopsToHit)
                {
                    ...
                    //resort
                    var pokestopListNearest =
                        pokestopList.OrderBy(
                        i =>
                            LocationUtils.CalculateDistanceInMeters(session.Client.CurrentLatitude,
                                session.Client.CurrentLongitude, i.Latitude, i.Longitude)).ToList();
                    var pokeStop = pokestopListNearest[0];
        ...
        private static async Task<List<FortData>> GetPokeStops(ISession session)
        {
            ...
            if(session.LogicSettings.Teleport)
                ...
                return mapObjects.MapCells.SelectMany(i => i.Forts)
                    .Where(
                    i =>
